...
/*
    ...
    println("=============== " + data_frame.rdd.partitions.size + "++++++++++++++++++++++++++++++++++++++=====================================")
...
//    println("-------------primary_zone COUNT " + primary_zone.count())
    ...
    println("Start: " + start_t1 + "      End: " + Calendar.getInstance().getTime())
        ...
        val city_aggregates_df = sqlContext.read.parquet(
          pathTemp + mode + "_mailru_primary_zone_city").groupBy($"city_id", $"interest_id").agg(
    ...
    //    println("--------- city_aggregates_df  COUNT: " + city_aggregates_df.count())
        ...
        city_aggregates_df.coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_mailru_agg_city")
...
*/
        ...
        val primary_aggregates = sqlContext.read.parquet(pathTemp + mode + "_mailru_primary_zone_city")
    ...
    //      .coalesce(8).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru_neighbour_raw")
        ...
        near_df.drop("near_zone_id", "pa_zone_id", "pa_mzone_id")
          .coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_mailru_neighbour_raw")

    println("  Start df_zone_id ...." + "\n")

        val df_zone_id = sqlContext.read.parquet(pathTemp + mode + "_mailru_neighbour_raw")
          .groupBy(col("zone_id"), col("interest_id"))
          .agg(max(col("avg_value")).alias("avg_value_neighbour_max"),
               round(avg(col("avg_value")), 3).cast("float").alias("avg_value_neighbour_mean"))

    println("  Finish df_zone_id ...." + "\n")
        ...
        */
        ...
        //
        df_zone_id.coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_mailru_neighbour_ready")
...
/*
        ...
        val prim_zone_city = sqlContext.read.parquet(pathTemp + mode + "_mailru_primary_zone_city")
        val city_agg = sqlContext.read.parquet(pathTemp + mode + "_mailru_agg_city")
    ...
    //    println("--------- primary_relative_agg  COUNT: " + primary_relative_agg.count())
        ...
        primary_relative_agg.coalesce(48).write.mode("overwrite").parquet(
          pathTemp + mode + "_mailru_primary_relative_agg")
       ...
       // ------------------prepare_final_dataset_before_pivot--------------------------------------------------------------------------
        ...
        val df_neighbour = sqlContext.read.parquet(pathTemp + mode + "_mailru_neighbour_ready")
          //.withColumn("mzone_id", col("zone_id") % 8)
          //.repartition(400,$"mzone_id")
        ...
        df_final_step1.coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_df_final_step1")
    ...
    //
        ...
        val df_final_step2 = sqlContext.read.parquet(pathTemp + mode + "_df_final_step1")
    ...
    //    println("--------- df_before_pivot  COUNT: " + df_before_pivot.count())
        ...
        //val start_t5 = Calendar.getInstance().getTime()
        df_before_pivot.coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_mailru")
    ...
    val df_before_pivot_2 = sqlContext.read.parquet(pathTemp + mode + "_mailru")
    ...
    df_pivot.coalesce(48).write.mode("overwrite").parquet(pathTemp + mode + "_final_mailru")

    println("  Processing process_pivot phase finished...." + "\n")
    println("Start: " + start_t6 + "      End: " + Calendar.getInstance().getTime() + "\n")


    println("Start_t1 primary_zone: " + start_t1 + "\n" + "     Start_t2: " + start_t2 +
