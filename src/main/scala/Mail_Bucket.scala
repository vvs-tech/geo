...
object Mail_Bucket {
  def main(args: Array[String]): Unit = {
    ...
    val path_p1 = "C:\\Programming\\vtb\\process_mailru\\geo\\mailru_portrait\\datep=2020-07-31"
    val path_p2 = "C:\\Programming\\vtb\\process_mailru\\geo\\mailru_portrait\\datep=2020-08-31"
    val path_p3 = "C:\\Programming\\vtb\\process_mailru\\geo\\mailru_portrait\\datep=2020-09-30"
...
/*
    ...
    primary_zone.coalesce(16).write.mode("overwrite").parquet(
    ...
    city_aggregates_df.coalesce(16).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru_agg_city")
    ...
    near_df.drop("near_zone_id", "pa_zone_id", "pa_mzone_id")
      .coalesce(16).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru_neighbour_raw")
    ...
    */
    ...
    //
    df_zone_id.coalesce(16).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru_neighbour_ready")
...
//~18 min
*/
//    val df_zone_id = sqlContext.read.parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru_neighbour_ready")
//    println("-------------- df_zone_id " +df_zone_id.count())
    ...
    primary_relative_agg.coalesce(16).write.mode("overwrite").parquet(
    ...
    //
    //
    val df_before_pivot = df_final_step1.join(
      broadcast(int_x_fdl), df_final_step1("interest_id")  === int_x_fdl("interest_id"), "right").select(
      df_final_step1("zone_id"),
      df_final_step1("city_id"),
      int_x_fdl("fieldname_cd").alias("interest_name"),
      df_final_step1("avg_value").alias("value"),
      df_final_step1("avg_value_city_relative").alias("city_relative"),
      df_final_step1("avg_value_neighbour_max").alias("neighbour_max"),
      df_final_step1("avg_value_neighbour_mean").alias("neighbour_mean"))
    ...
    df_before_pivot.coalesce(16).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_mailru")
    ...
    df_pivot.coalesce(16).write.mode("overwrite").parquet("C:\\Programming\\vtb\\process_mailru\\geo_tmp\\tmp_" + mode + "_final_mailru")
